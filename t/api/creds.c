...
START_TEST (creds_from_env_test) {
  ...
  ck_assert_msg(res < 0, "Failed to handle null creds");
  ck_assert_msg(errno == EINVAL, "Expected EINVAL (%d), got %s (%d)", EINVAL,
    strerror(errno), errno);

  res = pr_env_set(p, "AWS_ACCESS_KEY_ID", "FOO");
  ck_assert_msg(res == 0, "Failed to set AWS_ACCESS_KEY_ID env var: %s",
    strerror(errno));

  mark_point();
  res = aws_creds_from_env(p, &creds);
  ck_assert_msg(res < 0, "Failed to handle missing credentials");
  ck_assert_msg(errno == ENOENT, "Expected ENOENT (%d), got %s (%d)", ENOENT,
    strerror(errno), errno);

  res = pr_env_set(p, "AWS_SECRET_ACCESS_KEY", "BAR");
  ck_assert_msg(res == 0, "Failed to set AWS_SECRET_ACCESS_KEY env var: %s",
    strerror(errno));

  res = pr_env_set(p, "AWS_SESSION_TOKEN", "BAZ");
  ck_assert_msg(res == 0, "Failed to set AWS_SESSION_TOKEN env var: %s",
    strerror(errno));
  ...
  mark_point();
  res = aws_creds_from_env(p, &creds);
  ck_assert_msg(res == 0, "Failed to get credentials from env vars: %s",
    strerror(errno));

  expected = "FOO";
  ck_assert_msg(creds->access_key_id != NULL, "Expected access_key_id, got null");
  ck_assert_msg(strcmp(creds->access_key_id, expected) == 0,
    "Expected '%s', got '%s'", expected, creds->access_key_id);

  expected = "BAR";
  ck_assert_msg(creds->secret_access_key != NULL,
    "Expected secret_access_key, got null");
  ck_assert_msg(strcmp(creds->secret_access_key, expected) == 0,
    "Expected '%s', got '%s'", expected, creds->secret_access_key);

  expected = "BAZ";
  ck_assert_msg(creds->session_token != NULL,
    "Expected session_token, got null");
  ck_assert_msg(strcmp(creds->session_token, expected) == 0,
...
START_TEST (creds_from_file_test) {
  ...
  ck_assert_msg(res == 0, "Failed to write creds file '%s': %s", path,
    strerror(errno));

  mark_point();
  res = aws_creds_from_file(p, path, NULL, &creds);
  ck_assert_msg(res < 0, "Failed to handle badly formatted file");
  ck_assert_msg(errno == ENOENT, "Expected ENOENT (%d), got %s (%d)", ENOENT,
    strerror(errno), errno);
  ...
  mark_point();
  res = aws_creds_from_file(p, path, NULL, &creds);
  ck_assert_msg(res < 0, "Failed to handle file missing ID");
  ck_assert_msg(errno == ENOENT, "Expected ENOENT (%d), got %s (%d)", ENOENT,
    strerror(errno), errno);
  ...
  mark_point();
  res = aws_creds_from_file(p, path, NULL, &creds);
  ck_assert_msg(res < 0, "Failed to handle file missing secret");
  ck_assert_msg(errno == ENOENT, "Expected ENOENT (%d), got %s (%d)", ENOENT,
    strerror(errno), errno);
  ...
  mark_point();
  res = aws_creds_from_file(p, path, NULL, &creds);
  ck_assert_msg(res == 0, "Failed to handle properties file '%s': %s", path,
    strerror(errno));
...
START_TEST (creds_from_file_using_profile_test) {
  ...
  ck_assert_msg(res == 0, "Failed to write profile %s file '%s': %s",
    profile, path, strerror(errno));

  mark_point();
  res = aws_creds_from_file(p, path, profile, &creds);
  ck_assert_msg(res < 0, "Failed to handle malformed profiles file");
  ck_assert_msg(errno == ENOENT, "Expected ENOENT (%d), got %s (%d)", ENOENT,
    strerror(errno), errno);
  ...
  mark_point();
  res = aws_creds_from_file(p, path, profile, &creds);
  ck_assert_msg(res == 0, "Failed to handle profiles file '%s': %s", path,
    strerror(errno));
  ...
  expected = "quxxquzz";
  ck_assert_msg(creds->session_token != NULL, "Expected session_token, got null");
  ck_assert_msg(strcmp(creds->session_token, expected) == 0,
    "Expected '%s', got '%s'", expected, creds->session_token);
...
START_TEST (creds_from_iam_test) {
  ...
  ck_assert_msg(res < 0, "Failed to handle null iam_role");
  ck_assert_msg(errno == EINVAL, "Expected EINVAL (%d), got %s (%d)", EINVAL,
    strerror(errno), errno);
  ...
  ck_assert_msg(res < 0, "Failed to handle non-AWS environment");
...
START_TEST (creds_from_chain_test) {
  ...
  ck_assert_msg(res < 0, "Failed to handle null info");
  ck_assert_msg(errno == EINVAL, "Expected EINVAL (%d), got %s (%d)", EINVAL,
    strerror(errno), errno);
...
START_TEST (creds_from_sql_test) {
  ...
  ck_assert_msg(res < 0, "Failed to handle null query");
  ck_assert_msg(errno == EINVAL, "Expected EINVAL (%d), got %s (%d)", EINVAL,
    strerror(errno), errno);
  ...
  ck_assert_msg(res < 0, "Failed to handle invalid query '%s'", query);
